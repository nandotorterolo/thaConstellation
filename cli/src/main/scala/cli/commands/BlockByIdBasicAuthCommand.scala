...
class BlockByIdBasicAuthCommand[F[_]: Async: Console](cripto: Cripto[F], pathConfig: Path) {
  ...
  private def getBlock(blockId: BlockId, addressIdSigned: AddressIdSigned): CommandT[F, Unit] = {
    ...
    val request: Request[F] =
      Request[F]()
        .withUri(blockUri / blockId.value.toBase58)
...
object BlockByIdBasicAuthCommand {
  def apply[F[_]: Async: Console](implicit cripto: Cripto[F], pathConfig: Path): CommandT[F, Unit] =
    new BlockByIdBasicAuthCommand[F](cripto, pathConfig).command
}
